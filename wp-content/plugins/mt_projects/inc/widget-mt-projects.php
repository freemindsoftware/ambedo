...
// constructor
	...
	// Check values
		...
		$project_order 	= isset( $instance['project_order'] ) ? esc_attr( $instance['project_order'] ) : 'DESC';  
	...
	<!-- Style Selector -->
     ...
     <p>
      <label for="<?php echo $this->get_field_id('text'); ?>">Order: 
        <select class='widefat' id="<?php echo $this->get_field_id('project_order'); ?>"
                name="<?php echo $this->get_field_name('project_order'); ?>" type="text">
                
          <option value='DESC'<?php echo esc_attr(($project_order=='DESC'))?'selected':''; ?>>
            DESC
          </option>
          <option value='ASC'<?php echo esc_attr(($project_order=='ASC'))?'selected':''; ?>> 
            ASC
          </option>
	...
	<label for="<?php echo $this->get_field_id( 'random' ); ?>"><?php _e( 'Click to show in Random order', 'mtprojects' ); ?></label></p>
	...
	function update($new_instance, $old_instance) {
		...
		$instance['project_order'] 	= esc_attr($new_instance['project_order']);	
...
?>
        ...
        <?php endif; ?> 
            ...
            <div class="grid grid-pad overflow">		
                ...
                <div class="col-1-<?php if ( $columnset ) echo  $columnset; ?> mt-column-clear">  
                	<div class="project-box">
                    	<a href="<?php the_permalink(); ?>" class="transition-link">
			...
			<?php if ($see_all != '') : ?>
				...
				<a href="<?php echo esc_url($see_all); ?>" class="featured-link transition-link">  
                ...
                <div class="col-1-<?php if ( $columnset ) echo  $columnset; ?> mt-column-clear">  
                    ...
                    <a href="<?php the_permalink(); ?>" class="transition-link"> 
                    	...
                    	<?php if (has_post_thumbnail( get_the_ID() ) ): ?>
                    		...
                    		<div class="home-project-image-bg" style="background-image: url('<?php echo wp_get_attachment_url( get_post_thumbnail_id() , 'max-control' ); ?>');">
