	...
	if ( $style=='hover' ) : 	
		...
		while ( $scprojects->have_posts()) : $scprojects->the_post();
			...
			$output .= '<div class="col-1-' . $columns . ' mt-column-clear"><div class="project-box">';
  				...
  				$output .= '<a href="'.get_the_permalink().'" class="transition-link">';
	...
	if ( $style=='image' ) : 
		...
		$output .= '<section id="mt-projects"><div class="grid grid-pad">';
				...
				while ( $scprojects->have_posts()) : $scprojects->the_post();
					...
					$output .= '<div class="col-1-' . $columns . ' mt-column-clear venture-sc">';
		...
		while ( $scprojects->have_posts()) : $scprojects->the_post(); 
			...
			$output .= '<div class="col-1-' . $columns . ' mt-column-clear"><div class="single-work">';
				...
				$output .= '<span>'; 
					...
					$output .= '<h5><a href="'.get_the_permalink().'" class="transition-link">'.get_the_title().'</a></h5>';    
					
					$output .= '<a href="'.get_the_permalink().'" class="transition-link"><i class="fa fa-long-arrow-right"></i></a>';   
